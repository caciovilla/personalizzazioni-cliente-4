...
let $Avviso003 := $r//ns4:Postalizzazione[ns4:TipoPostalizzazione = "003"]
let $Avviso007 := $r//ns4:Postalizzazione[ns4:TipoPostalizzazione = "007"]
let $Avviso008 := $r//ns4:Postalizzazione[ns4:TipoPostalizzazione = "008"]    
...
group by $idLotto
return
<Res idLotto="{$idLotto}" tipoDocumento="{distinct-values($tipoDocumento)}" nCartelle="{count($r)}" totaleEstremiDocumento="{sum($TotaleDaVersareEstremiDoc)}" nVirtualIban="{count($cVIban)}" >
<contatori>
  ...
  <TipoAvviso>
    <Avviso003>{count($Avviso003)}</Avviso003>
    <Avviso007>{count($Avviso007)}</Avviso007>
    <Avviso008>{count($Avviso008)}</Avviso008>
  </TipoAvviso>
<differenze>
